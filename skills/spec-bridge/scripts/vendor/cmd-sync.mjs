...
export async function run(args, {
  ...
  if (existsSync(changesDir)) {
    for (const dir of readdirSync(changesDir)) {
      ...
      if (!isActiveChange) {
        if (!existsSync(join(dirPath, '.bridge.yaml'))) continue;
        const otherStage = readState(dirPath).stage;
        if (otherStage === 'archived' || otherStage === 'abandoned') continue;
      }
  ...
  if (existsSync(join(changeDir, '.bridge.yaml'))) {
    ...
    stdout.write('  🧾 Wrote publication receipt to .bridge.yaml\n');
  }
